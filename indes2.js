...
function processFirstItem(stringList, callback) {
  ...
  */
  function processDuplicateFree(list, callback) {
    const duplicate = list.filter(function(list, callback){
      return list.indexOf(callback) === callback;
    }) 
  }
  ...
  */
  function lowerCaseStrings(strings) {
    const short = [];
  strings.forEach(function(good){
   short.push(good.toLowerCase());
  });
  return short;
  }
  ...
  */
  function isItAnApple(strings) {
   return strings.map(function (fruit){
     return fruit === 'apple';
   })
  }
  ...
  */
  function removeApple(strings) {
    return strings.filter(function(fruits){
      return fruits !== 'apple';
    });
  }
  ...
  */
  function stringSmash(strings) {
    const together = strings.reduce(function(string1, string2){
